...
const TABLE_HEAD = [
  {
    id: "ediStdNomenclature",
    label: "Edi Std Nomen Clature",
    alignRight: false,
  },
  ...
  {
    id: "orderArrivedInFull",
    label: "Order Arrived In Full",
    alignRight: false,
  },
...
export default function Users(props) {
  ...
  const { inventory, isInventoryLoading } = useSelector(
  ...
  const canAddInventory = hasPermission(
    userPermissions,
    PERMISSIONS.ADD_INVENTORY
  );
  const canEditInventory = hasPermission(
    userPermissions,
  ...
  const canExportInventory = hasPermission(
    userPermissions,
    PERMISSIONS.EXPORT_INVENTORY
  );
  const canViewInventoryList = hasPermission(
    userPermissions,
  ...
  function applySortFilter(array, comparator, query) {
    ...
    if (query) {
      return filter(
        inventory,
        (_inv) =>
          _inv.ediStdNomenclature.toLowerCase().indexOf(query.toLowerCase()) !==
          -1
      );
    }
                        ...
                        return (
                          <TableRow hover key={id} tabIndex={-1}>
                            ...
                            <TableCell align="left">
                              <Label
                                color={
                                  orderArrivedInFull === true
                                    ? "success"
                                    : "error"
                                }
                              >
                                {sentenceCase(
                                  orderArrivedInFull ? "Yes" : "No"
                                )}
                              </Label>
                            </TableCell>
                            <TableCell align="left">
