...
import { deletePermission, getPermission } from '../../feature/permissionsSlice';
...
export default function NewPermissions(props) {
  ...
  const { permissions, isPermissionLoading } = useSelector((state) => state.permissionSlice);
  const { loginUser } = useSelector((state) => state.userSlice);

  const { permissions: userPermissions } = loginUser.decodedToken;
  const canAddPermission = hasPermission(userPermissions, PERMISSIONS.ADD_PERMISSION);
  const canEditPermission = hasPermission(userPermissions, PERMISSIONS.EDIT_PERMISSION);
  const canDeletePermission = hasPermission(userPermissions, PERMISSIONS.DELETE_PERMISSION);
  const canViewPermissionList = hasPermission(userPermissions, PERMISSIONS.VIEW_PERMISSION);
  ...
  function applySortFilter(array, comparator, query) {
    ...
    if (query) {
      return filter(permissions, (_perm) => _perm.name.toLowerCase().indexOf(query.toLowerCase()) !== -1);
    }
  ...
  const filteredUsers = applySortFilter(permissions, getComparator(order, orderBy), filterName);
  ...
  return (
    <>
      ...
      {openModel ? (
        <CModel isLoading={isPermissionLoading} open={openModel} setOpen={setOpenModel} filter={'add-permission'} />
      ) : (
        ''
      )}
      {openUpdateModel ? (
                        ...
                        <TableRow>
                          ...
                          <TableCell align="left">
                            <Label color={isActive === true ? 'success' : 'error'}>
                              {sentenceCase(isActive ? 'active' : 'In Active')}
                            </Label>
                          </TableCell>
